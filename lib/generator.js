var path      = require( 'path' )
  ...
  , generator = require( path.join( __dirname, 'generate' ) );
...
function generateRun( projects, tmpls, names, fn ) {
  async.each( projects, function ( project, next ) {
    generator.fs.touchDirs( project, tmpls, names )
    .then( function ( dirs ) {
      return generator.fs.checkFiles( project, dirs, tmpls, names ).then( function( ) {

      } ).return( dirs );
    } )
    .then( function ( dirs ) {
      generator.render.template( dirs, names, tmpls, project, function ( err, files ) {
        if (!!err) {
          return fn( err );
        }
...
exports.generate = function( project, tmpls, names, fn ) {
  ...
  if (!!project) {
    ...
    util.locations.get( true ).then( function ( projects ) {
