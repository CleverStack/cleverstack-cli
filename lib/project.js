...
var installBundledModules = exports.installBundledModules = function ( project, deps, projectDir ) {
  ...
  async.filter( deps, function ( dep, cb ) {
    ...
    var def   = Promise.defer( )
      , proc  = spawn( path.join( __dirname, '..', 'bin', 'clever-install' ), _deps, { cwd: _path, stdio: 'inherit' } );
...
function runReadMe ( projectFolder ) {
  ...
  var args = [ '--base', projectFolder, '--gruntfile', path.resolve( path.join( projectFolder, 'Gruntfile.js' ) ), 'readme' ];
  var proc = spawn( 'grunt', args, { cwd: projectFolder, stdio: 'inherit' } );
...
function runDBMigrations ( projectFolder ) {
  ...
  var args = [ '--base', projectFolder, '--gruntfile', path.resolve( path.join( projectFolder, 'Gruntfile.js' ) ), 'db' ]
    , env  = process.env;
...
exports.installModule = function ( project, modulePath ) {
  ...
  function ( err ) {
    ...
    if (!fs.existsSync( bowerPath ) ) {
      // do we need to run db migrations?
      readGruntTasks( modulePath )
      .spread( function ( tasks, __path__ ) {
        var actions = [ ];

        if (tasks.indexOf( 'db' ) > -1) {
          actions.push( runDBMigrations( projectFolder ) );
        }

        if (tasks.indexOf( 'readme') > -1) {
          actions.push( runReadMe( projectFolder ) );
        }
