...
function runGruntTask ( projectFolder, cmd ) {
  var def   = Promise.defer( )
...
function runReadMe ( projectFolder ) {
  return runGruntTask( projectFolder, 'readme' );
}
...
function runPrompt ( projectFolder ) {
  return runGruntTask( projectFolder, 'prompt:clever' );
}
...
function runDBMigrations ( projectFolder ) {
  ...
  exec( 'NODE_ENV=' + env.NODE_ENV + ' grunt db', { env: env, cwd: projectFolder }, function ( err, stderr ) {
    if (!!err) {
  ...
  proc.on( 'close', function ( code ) {
    ...
    function ( modules, err ) {
      ...
      async.each( modules, function ( moduleDir, fn ) {
        ...
        exec( 'bower ' + [ '-F', '--config.cwd=' + path.join( projectFolder.moduleDir, projectFolder.modulePath, moduleDir ), '--config.directory=' + path.relative( path.join( projectFolder.moduleDir, projectFolder.modulePath, moduleDir ), [ projectFolder.moduleDir ].concat( _bowerRC.directory.split( '/' ) ).join( path.sep ) ), 'install' ].join( ' ' ), { env: process.env, cwd: path.join( projectFolder.moduleDir, projectFolder.modulePath, moduleDir ) }, function ( _err, _stderr ) {
          if (!!err || _stderr.trim( ) !== "") {
            return fn( err || _stderr.trim( ) );
          }
